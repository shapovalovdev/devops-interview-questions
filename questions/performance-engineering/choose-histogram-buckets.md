...
- Every bucket is a separate time series, multiplied by every label combination, and combinations multiply: twelve buckets across 30 endpoints, 5 methods, and 4 status classes is 7,200 series per instance before `_sum` and `_count`. Boundaries are also a compatibility contract — changing them makes old and new series non-mergeable and breaks any comparison spanning the change, and `le` values must be formatted identically across services or the aggregation silently splits. Native histograms (Prometheus 2.40 and later, still experimental) replace the choice with a resolution factor but need a matching exposition and query stack.
