...
- Other MVCC engines have the same long-transaction disease with different symptom names: InnoDB exposes it as history-list-length growth behind purge threads, and SQL Server's row-versioning stores old versions in tempdb — a stale snapshot blocking cleanup is an engine-neutral failure mode.
