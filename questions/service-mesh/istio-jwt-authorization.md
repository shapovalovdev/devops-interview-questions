...
- Request-authentication placement varies by mesh: Linkerd validates no JWTs itself (that stays in the application or an external authorization service), while Gateway API implementations expose JWT validation through their own policy extensions, such as Envoy Gateway's SecurityPolicy — authenticate-then-authorize maps, but the component differs.
