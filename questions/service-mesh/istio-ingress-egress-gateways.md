...
- The north-south half is standardized: Gateway API Gateway and HTTPRoute objects are implemented by Istio, Envoy Gateway, and Cilium alike, so an ingress boundary can migrate between controllers — egress enforcement stays mesh- or CNI-specific, as with Cilium's egress gateway with fixed source IPs.
