...
- The constraint people miss is visibility. Configuration is backend-only by default, and a key reaches the browser only when its schema marks it as frontend-visible, which is why a value the UI needs can look correctly set and still be missing in the page. How that frontend configuration is delivered depends on how you serve the app: the documented Kubernetes deployment bundles the frontend into the backend image and serves it from there, so a separate static frontend is a different problem with different build-time inputs. Backend-only values, including database credentials and integration tokens, should arrive as environment variables from a secret store rather than being written into a committed file.
