...
def test_browser_installation_has_a_bounded_actionable_failure() -> None:
    ...
    assert "timeout --foreground 8m python -m playwright install --with-deps chromium" in install
    assert "Chromium installation exceeded its 8-minute limit" in install
