"""Focused acceptance checks for GitHub issue #81's Caching Questions."""
...
def test_caching_set_is_complete_and_source_verified() -> None:
    tags = known_tags()
...
def test_caching_related_materials_has_five_curated_links() -> None:
    related = ROOT / "docs" / "related-materials" / f"{THEME}.md"
    assert len(resource_links(related.read_text(encoding="utf-8"), str(related))) == 5


def test_every_caching_question_is_registered_for_the_link_audit() -> None:
    audited = {
        item["question"]
...
def test_caching_is_declared_complete_and_registered_in_the_catalog() -> None:
    import json
...
def test_caching_does_not_restate_the_backend_architecture_http_cache_questions() -> None:
    reserved = {
    ...
    for path in sorted(QUESTIONS.glob("*.md")):
        for line in path.read_text(encoding="utf-8").splitlines():
            if line.startswith("title: "):
                ...
                break
    assert not titles & reserved, f"{THEME} restates a backend-architecture Question: {sorted(titles & reserved)}"
