"""Acceptance checks for GitHub issue #70's ci-cd learning resources.

Every active ci-cd Question must parse under the curated-resource schema and must
be registered in the link-audit manifest alongside the Theme's related-materials
page.
"""
...
def ci_cd_questions() -> list[Path]:
    questions = sorted((ROOT / "questions" / THEME).glob("*.md"))
    assert questions, "no ci-cd Questions found"
    return questions
...
def test_every_ci_cd_question_has_five_curated_links() -> None:
    for question in ci_cd_questions():
        ...
        assert all(url.startswith("https://") for _, url in links), f"{question}: links must use HTTPS"


def test_ci_cd_related_materials_page_is_curated() -> None:
...
def test_every_ci_cd_question_is_registered_in_the_manifest() -> None:
