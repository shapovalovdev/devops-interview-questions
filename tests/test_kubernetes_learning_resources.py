"""Acceptance checks for GitHub issue #69's Kubernetes learning resources.

Every active kubernetes Question must parse under the curated-resource schema
...
def kubernetes_questions() -> list[Path]:
    questions = sorted((ROOT / "questions" / THEME).glob("*.md"))
    assert questions, "no kubernetes Questions found"
    return questions
...
def test_every_kubernetes_question_has_five_curated_links() -> None:
    for question in kubernetes_questions():
...
def test_kubernetes_related_materials_page_is_curated() -> None:
...
def test_every_kubernetes_question_is_registered_in_the_manifest() -> None:
