...
The official scope names two objectives that were not addressed at the required
configuration depth. The following original canonical Questions fill them;
their `ckne` tags become publicly filterable only during central catalog,
manifest, vocabulary, and validator integration.

1. [**Configure and troubleshoot a multi-interface Pod.**](../../questions/container-networking/multus-multi-interface-pod-troubleshooting.md)
   It is in the `container-networking` Theme and is verified against the upstream
   [Multus CNI documentation](https://github.com/k8snetworkplumbingwg/multus-cni/tree/master/docs).
   It covers default-route selection, NetworkAttachmentDefinition ownership,
   IPAM/routing validation, and the blast radius of changing a live workload's
   network attachments.
2. [**Route LLM inference traffic with Gateway API controls.**](../../questions/service-mesh/gateway-api-llm-inference-routing.md)
   It is in the `service-mesh` Theme and is verified against the upstream
   [Gateway API Inference Extension documentation](https://gateway-api-inference-extension.sigs.k8s.io/).
   It distinguishes extension/controller constraints from core Gateway API,
   covers routing/endpoint signals and safe fallback, and does not claim
   portable LLM traffic semantics.
